...
const Overview = (() => {
  ...
  function focusCoordinates(point) {
    const lat = Number(point?.lat);
    const lon = Number(point?.lon);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return;
    zoomToPoint({ lat, lon });
    window.dispatchEvent(new CustomEvent('overview:focus-changed', {
      detail: { lat, lon, label: point.label || 'signal origin' }
    }));
  }
  ...
  function getVisibleContext() {
    return [...layers].map(([id, layer]) => ({
      ...getLayerSnapshot(id),
      points: layer.points.filter(point => pointIsRendered(point, id))
    }));
  }
