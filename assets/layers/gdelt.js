...
window.OverviewNewsGeo = Object.freeze({
  centroid(country) {
    const point = GDELT_COUNTRY_CENTROIDS[GDELT_COUNTRY_ALIASES[String(country || '').trim()] || gdeltCountryKey(country)];
  ...
  async fetchPoints() {
    ...
    try {
       const params = new URLSearchParams({
         query: 'breaking OR emergency OR conflict OR crisis OR protest OR disaster OR earthquake OR wildfire OR flood OR outbreak OR election',
         mode: 'artlist',
         format: 'jsonp',
         maxrecords: '250',
         timespan: '24h',
       ...
       const articlesWithOrigin = articles.map(article => {
         ...
         return {
           ...article,
           sourceLat: centroid?.[0] ?? null,
           sourceLon: centroid?.[1] ?? null
         };
       });
       const points = articlesWithOrigin.map(article => {
         const country = String(article.sourcecountry || '').trim();
         const centroid = Number.isFinite(article.sourceLat) && Number.isFinite(article.sourceLon)
           ? [article.sourceLat, article.sourceLon]
           : null;
