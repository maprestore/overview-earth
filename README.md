...
One dark, readable surface for early global news signals, earthquakes, satellites, aircraft, ships, and submarine cables.
...
The Planetary Copilot is deliberately evidence-bound. It uses the observations, filters, source states, timestamps, replay mode, and provenance already loaded in the current browser. Every answer separates:

- **Known**: facts directly present in the loaded sources.
- **Unknown**: unavailable feeds, missing timestamps, or gaps in coverage.
- **Not established**: claims the data does not prove, such as causality or event location from a reporting-country centroid.

The copilot exports the same context as a time-ordered Evidence Ledger, so another person can inspect the source posture instead of trusting an opaque answer. A future hosted deployment may add a model relay behind the same contract; the open build never sends credentials or invents a conclusion when no model is configured.
...
| Global News Radar | A GDELT-powered 24-hour reporting radar with fresh-signal filters, headline search, coverage sorting, worldwide metrics, source domains, map focus controls, and safe outbound links. It clearly separates reporting origin from event location. |
| Story Mode | One-click views for a live baseline, impact signals, Worldwatch early coverage, infrastructure, or the built world. Each view is a shareable URL. |
...
| Evidence dossier | Persistent metadata, confidence and freshness badges, location focus, provenance chain, nearby source-bounded context, evidence copying, source links, and a local watchlist that works with mouse, touch, and keyboard input. |
| Planetary Copilot | A local, evidence-bound analysis layer that separates known facts, unknowns, and claims not established by the loaded data, then exports a reproducible time-ordered ledger. |
...
6. **Try Worldwatch.** See the early global news radar with freshness filters, coverage metrics, and a central-map ticker.
7. **Try Built World.** See live OpenStreetMap building footprints and directional aircraft glyphs instead of generic dots.
8. **Share the view.** Copy the current URL and send someone the exact same layer selection, filters, or replay state.
9. **Use the command deck.** Focus a region, find a loaded signal by label, or export the current observations for further analysis.
10. **Open Mission Console.** Try “strong earthquakes near Japan,” scrub the Time Machine, or open an evidence brief from a fused signal.
11. **Share the data.** Export history as JSON, GeoJSON, or CSV, or use a replay pack to reproduce a known scene without live feeds.
...
| News | Experimental | [GDELT DOC 2.0](https://www.gdeltproject.org/) | Up to 250 recent reporting signals across a 24-hour query, with freshness filters and reporting-country centroids. [Query](https://api.gdeltproject.org/api/v2/doc/doc?query=breaking%20OR%20emergency%20OR%20conflict%20OR%20crisis%20OR%20protest%20OR%20disaster%20OR%20earthquake%20OR%20wildfire%20OR%20flood%20OR%20outbreak%20OR%20election&mode=artlist&format=json&maxrecords=250&timespan=24h&sort=datedesc) |
